function predictions = causalpreventive(master_obs,master_n,data,bootflag)

% CAUSALPREVENTIVE   Computes causal support for a preventive relation
%
% Usage:
%
%   PREDICTIONS = CAUSALPREVENTIVE(MASTER_OBS,MASTER_N,DATA,BOOTFLAG)
%
...
master_deltap = -1*(master_p(:,1)-master_p(:,2));
master_cheng = master_deltap./(master_p(:,2)+eps);
master_chi2 = zeros(n,1);
master_pci = zeros(n,1);
master_bayes = zeros(n,1);
warning off
for trial = 1:n % D is C,B,E
  D(1,2,2) = master_obs(trial,2);
  D(2,2,2) = master_obs(trial,1);
  D(1,2,1) = master_n(trial,2)-D(1,2,2); 
  D(2,2,1) = master_n(trial,1)-D(2,2,2); 
  D(:,1,:) = 0;
  master_bayes(trial) = supportsampler(master_obs(trial,:),master_n(trial,:),500000,-1);
...
predictions{4}.name = 'Support (AND-NOT)';
